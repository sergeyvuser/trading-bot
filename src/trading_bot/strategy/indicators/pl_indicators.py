...
def ema_exprs(timeperiod: int, column: str = "close", **_) -> list[pl.Expr]:
    return [
        plta.ema(real=pl.col(column), timeperiod=timeperiod).alias(f"ema_{timeperiod}")
    ]


def sma_exprs(timeperiod: int, column: str = "close", **_) -> list[pl.Expr]:
    return [
        plta.sma(real=pl.col(column), timeperiod=timeperiod).alias(f"sma_{timeperiod}")
    ]


def rsi_exprs(timeperiod: int = 14, column: str = "close", **_) -> list[pl.Expr]:
    return [
        plta.rsi(real=pl.col(column), timeperiod=timeperiod).alias(f"rsi_{timeperiod}")
    ]
...
def atr_exprs(timeperiod: int = 14, **_) -> list[pl.Expr]:
    return [
        plta.atr(
            high=pl.col("high"),
            low=pl.col("low"),
